...
def _gemini_client():
    """Return a generate_content-compatible model (Gemini or OpenRouter fallback)."""
    # Import lazily so the rest of the module still loads if LLM deps are missing.
    import sys
    root = Path(__file__).resolve().parent.parent
    if str(root) not in sys.path:
        sys.path.insert(0, str(root))
    from llm_bridge import get_text_model
    return get_text_model("gemini-2.5-flash")
