...
console.log("QMOOSA MASTER PROJECT FINISHER — JARSOL WEB4 AUTOMATON");
...
command("secrets & mainnet fail-closed safety", "npm", ["run", "verify:secrets"], { timeout: 180000 });
...
command("quantum portfolio & research verification", "npm", ["run", "test:quantum"], { timeout: 180000 });
command("reality manifest contract tests", "npm", ["run", "test:manifest"], { timeout: 180000 });
command("URS 10-gate universal reality audit", "npm", ["run", "reality:universal"], { timeout: 180000 });
command("solana devnet on-chain verification", "npm", ["run", "verify:devnet"], { timeout: 180000 });
command("solana testnet canonical verification", "npm", ["run", "verify:testnet"], { timeout: 180000 });
command("typescript strict typecheck", "npm", ["run", "typecheck"], { timeout: 180000 });
...
try {
  const ignoredDynamicPatterns = [
    ...
    "jarsol-deployment.json"
  ];
  const porcelain = execFileSync("git", ["status", "--porcelain"], { cwd: root, encoding: "utf8" })
    .trim()
    .split("\n")
    .map(l => l.trim())
    .filter(l => l && !ignoredDynamicPatterns.some(pattern => l.includes(pattern)))
    .join("\n");
...
const deployUrl = process.env.QMOOSA_DEPLOYMENT_URL || "https://elon00.github.io/jarsol-web4-automaton/";
...
try {
  ...
  const hasAppDom = html.includes('id="root"') || html.includes('assets/index') || html.includes('JarSol');
